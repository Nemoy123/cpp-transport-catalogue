...
public:
    ...
    //explicit Router(Router&& other_router);
    Router& operator=(Router&& other_router)
    {
        graph_ = other_router.graph_;
        routes_internal_data_ = std::move(other_router.routes_internal_data_);
        //ZERO_WEIGHT  = std::move(other_router.ZERO_WEIGHT);
